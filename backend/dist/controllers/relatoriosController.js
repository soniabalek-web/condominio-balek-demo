...
exports.gerarRelatorioKondor = exports.gerarRelatorioSindico = exports.gerarRelatorioDevedores = exports.gerarRelatorioHistoricoApartamento = exports.gerarRelatorioDespesasParceladas = exports.gerarRelatorioConfiguracoes = exports.gerarRelatorioGas = exports.gerarRelatorioDespesas = exports.gerarExtratoBank = void 0;
...
const buscarDadosCondominio = async () => {
    const configResult = await database_1.default.query(`SELECT chave, valor FROM configuracoes
...
const gerarExtratoBank = async (req, res) => {
    try {
        ...
        const [saldoResult, transacoesResult, dataInicialResult, dataFinalResult] = await Promise.all([
            database_1.default.query('SELECT * FROM banco_saldo WHERE mes = $1 AND ano = $2', [mes, ano]),
            database_1.default.query(`SELECT bt.*, cd.nome as categoria_nome
         FROM banco_transacoes bt
         LEFT JOIN categorias_despesas cd ON bt.categoria_id = cd.id
         WHERE bt.mes = $1 AND bt.ano = $2
         ORDER BY bt.data_transacao, bt.criado_em`, [mes, ano]),
            database_1.default.query(`SELECT MAX(data_transacao) as ultima_data FROM banco_transacoes WHERE mes = $1 AND ano = $2`, [mesAnteriorNum, anoAnteriorNum]),
            database_1.default.query(`SELECT MAX(data_transacao) as ultima_data FROM banco_transacoes WHERE mes = $1 AND ano = $2`, [mes, ano])
        ]);
        ...
        if (transacoesResult.rows.length > 0) {
            const datas = transacoesResult.rows.map((t) => new Date(t.data_transacao));
        ...
        const saldoAnteriorResult = await database_1.default.query('SELECT saldo_final FROM banco_saldo WHERE mes = $1 AND ano = $2', [mesAnterior, anoAnterior]);
        ...
        transacoesResult.rows.forEach((transacao, index) => {
...
exports.gerarExtratoBank = gerarExtratoBank;
// Relatório de Despesas do Condomínio
const gerarRelatorioDespesas = async (req, res) => {
    try {
        const { mes, ano } = req.params;
        // Buscar despesas
...
exports.gerarRelatorioDespesas = gerarRelatorioDespesas;
// Relatório de Consumo de Gás (12 meses)
const gerarRelatorioGas = async (req, res) => {
    try {
       ...
       LIMIT 72`);
        ...
        Object.values(periodos).forEach((periodo, index) => {
            ...
            y += 20;
            periodo.leituras.forEach((leitura, idx) => {
...
exports.gerarRelatorioGas = gerarRelatorioGas;
// Relatório de Configurações
const gerarRelatorioConfiguracoes = async (req, res) => {
    try {
        const dadosCondominio = await buscarDadosCondominio();
        // Buscar todas as configurações
        const [configResult, condominosResult, emailsResult, cnpjResult] = await Promise.all([
            database_1.default.query('SELECT * FROM configuracoes ORDER BY chave'),
            database_1.default.query('SELECT * FROM condominos ORDER BY apartamento'),
            database_1.default.query('SELECT * FROM emails_permitidos ORDER BY apartamento'),
            database_1.default.query("SELECT valor FROM configuracoes WHERE chave = 'cnpj_condominio'")
        ]);
        ...
        y += 25;
        const dadosCadastro = {
            nome: configResult.rows.find((c) => c.chave === 'cond_nome')?.valor || 'N/C',
            cnpj: configResult.rows.find((c) => c.chave === 'cnpj_condominio')?.valor || 'N/C',
            endereco: configResult.rows.find((c) => c.chave === 'cond_endereco')?.valor || 'N/C',
            numero: configResult.rows.find((c) => c.chave === 'cond_numero')?.valor || '',
            complemento: configResult.rows.find((c) => c.chave === 'cond_complemento')?.valor || '',
            bairro: configResult.rows.find((c) => c.chave === 'cond_bairro')?.valor || 'N/C',
            cidade: configResult.rows.find((c) => c.chave === 'cond_cidade')?.valor || 'N/C',
            estado: configResult.rows.find((c) => c.chave === 'cond_estado')?.valor || 'N/C',
            cep: configResult.rows.find((c) => c.chave === 'cond_cep')?.valor || 'N/C',
            nomeSindico: configResult.rows.find((c) => c.chave === 'cond_nome_sindico')?.valor || 'N/C',
            telSindico: configResult.rows.find((c) => c.chave === 'cond_tel_sindico')?.valor || 'N/C',
            emailCondominio: configResult.rows.find((c) => c.chave === 'cond_email_condominio')?.valor || 'N/C',
            emailSindico: configResult.rows.find((c) => c.chave === 'cond_email_sindico')?.valor || 'N/C'
        };
        ...
        y += 25;
        const fundoReservaConfig = configResult.rows.filter((c) => c.chave.includes('fundo_reserva'));
        doc.rect(40, y, 515, 20).fill('#E3F2FD');
        doc.fillColor('#1565C0').fontSize(9).font('Helvetica-Bold')
            .text('Configuração', 45, y + 6)
            .text('Valor', 400, y + 6, { width: 145, align: 'right' });
        y += 20;
        fundoReservaConfig.forEach((config, index) => {
            const bgColor = index % 2 === 0 ? '#FFFFFF' : '#F5F5F5';
        ...
        doc.fillColor('#1565C0').fontSize(9).font('Helvetica-Bold')
            ...
            .text('Email', 460, y + 6);
        y += 20;
        condominosResult.rows.forEach((cond, index) => {
            const bgColor = index % 2 === 0 ? '#FFFFFF' : '#F5F5F5';
            doc.rect(40, y, 515, 20).fill(bgColor);
            doc.fillColor('#333333').fontSize(8).font('Helvetica')
...
exports.gerarRelatorioConfiguracoes = gerarRelatorioConfiguracoes;
// Manter a função de despesas parceladas para compatibilidade (redireciona para configurações)
exports.gerarRelatorioDespesasParceladas = exports.gerarRelatorioConfiguracoes;
// Relatório de Histórico por Apartamento (12 meses)
const gerarRelatorioHistoricoApartamento = async (req, res) => {
    try {
        const dadosCondominio = await buscarDadosCondominio();
        // Buscar todos os apartamentos
        const condominosResult = await database_1.default.query('SELECT * FROM condominos ORDER BY apartamento');
        // Buscar CNPJ
        const cnpjResult = await database_1.default.query("SELECT valor FROM configuracoes WHERE chave = 'cnpj_condominio'");
        const cnpj = cnpjResult.rows[0]?.valor || '49.936.617/0001-02';
        ...
        for (const condomino of condominosResult.rows) {
            ...
            const mesesResult = await database_1.default.query(`SELECT DISTINCT mes, ano
         ...
         LIMIT 12`);
            ...
            for (const mesAno of mesesResult.rows) {
                ...
                const despesasResult = await database_1.default.query(`SELECT SUM(valor) as total_despesas
           FROM despesas_condominio
           WHERE mes = $1 AND ano = $2`, [mes, ano]);
                ...
                const gasResult = await database_1.default.query(`SELECT valor_total
           FROM leituras_gas
           WHERE mes = $1 AND ano = $2 AND apartamento = $3`, [mes, ano, apartamento]);
            ...
            doc.fillColor('#666666').fontSize(8).font('Helvetica')
                ...
                : `Fundo de Reserva: ${percentual}% sobre o valor do condomínio`, 50, y + 12);
            // Rodapé
            doc.fillColor('#666666').fontSize(8).font('Helvetica')
                .text(`Relatório gerado em ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}`, 40, 750, { align: 'center' });
...
exports.gerarRelatorioHistoricoApartamento = gerarRelatorioHistoricoApartamento;
// Relatório de Devedores
const gerarRelatorioDevedores = async (req, res) => {
    try {
        const dadosCondominio = await buscarDadosCondominio();
        const result = await database_1.default.query(`SELECT * FROM pagamentos_moradores
       WHERE pago = false
       ORDER BY ano DESC, mes DESC, apartamento`);
        ...
        if (result.rows.length === 0) {
            ...
            Object.keys(devedoresPorApto).sort().forEach((apto) => {
                const debitos = devedoresPorApto[apto];
                const totalDevido = debitos.reduce((sum, d) => sum + parseFloat(d.valor_total), 0);
...
exports.gerarRelatorioDevedores = gerarRelatorioDevedores;
// Relatório Completo para o Síndico
const gerarRelatorioSindico = async (req, res) => {
    try {
        const dadosCondominio = await buscarDadosCondominio();
        const { mes, ano } = req.params;
        ...
        const [saldoResult, transacoesResult, despesasResult, gasResult, condominosResult, dataInicialResult, dataFinalResult] = await Promise.all([
         ...
         WHERE bt.mes = $1 AND bt.ano = $2
         ORDER BY bt.data_transacao, bt.criado_em`, [mes, ano]),
            database_1.default.query(`SELECT dc.*, cd.nome as categoria_nome, cd.ordem
         FROM despesas_condominio dc
         LEFT JOIN categorias_despesas cd ON dc.categoria_id = cd.id
         WHERE dc.mes = $1 AND dc.ano = $2
         ORDER BY cd.ordem, dc.descricao`, [mes, ano]),
            database_1.default.query(`SELECT * FROM leituras_gas WHERE mes = $1 AND ano = $2 ORDER BY apartamento`, [mes, ano]),
            database_1.default.query(`SELECT apartamento, nome_proprietario as nome, email FROM condominos ORDER BY apartamento`, []),
            // Buscar última data de transação do mês anterior
            database_1.default.query(`SELECT MAX(data_transacao) as ultima_data
         FROM banco_transacoes
         WHERE mes = $1 AND ano = $2`, [mesAnteriorNum, anoAnteriorNum]),
        ...
        const sindico = condominos.find((c) => c.apartamento === '05');
        ...
        for (let i = 1; i <= 6; i++) {
            ...
            const condomino = condominos.find((c) => c.apartamento === apto);
            const gasApto = gasResult.rows.find((g) => g.apartamento === apto);
        ...
        if (fundoReserva > 0) {
            ...
            doc.fontSize(9).font('Helvetica').fillColor('#666666')
                ...
                : `${percentual}% sobre o valor do condomínio: ${formatarMoeda(fundoReserva)}`, 50, y + 28);
        }
        // Rodapé página 1
        doc.fillColor('#666666').fontSize(8).font('Helvetica')
            .text(`Relatório gerado em ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}`, 40, 750, { align: 'center' });
        ...
        if (gasResult.rows.length > 0) {
            ...
            gasResult.rows.forEach((gas, index) => {
...
exports.gerarRelatorioSindico = gerarRelatorioSindico;
// Relatório Mensal para Kondor Imóveis
const gerarRelatorioKondor = async (req, res) => {
    try {
        const dadosCondominio = await buscarDadosCondominio();
        const { mes, ano } = req.params;
        // Buscar todas as informações necessárias
        const [despesasResult, gasResult, condominosResult] = await Promise.all([
         ...
         ORDER BY cd.ordem, dc.descricao`, [mes, ano]),
            database_1.default.query(`SELECT * FROM leituras_gas WHERE mes = $1 AND ano = $2 ORDER BY apartamento`, [mes, ano]),
            database_1.default.query(`SELECT apartamento, nome_proprietario as nome, nome_morador, email FROM condominos ORDER BY apartamento`, [])
        ]);
        const condominos = condominosResult.rows;
        // Buscar CNPJ das configurações
        const cnpjResult = await database_1.default.query("SELECT valor FROM configuracoes WHERE chave = 'cnpj_condominio'");
...
exports.gerarRelatorioKondor = gerarRelatorioKondor;
