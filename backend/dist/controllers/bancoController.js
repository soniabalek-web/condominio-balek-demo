...
exports.mudarMesCobranca = exports.conferirSaldo = exports.calcularSaldo = exports.excluirTransacao = exports.atualizarTransacao = exports.listarTransacoes = exports.criarTransacao = exports.obterSaldoMensal = exports.salvarSaldoMensal = void 0;
const database_1 = __importDefault(require("../config/database"));
// Criar/Atualizar saldo mensal
const salvarSaldoMensal = async (req, res) => {
    try {
        const { mes, ano, saldo_inicial, saldo_extrato } = req.body;
        if (!mes || !ano || saldo_inicial === undefined) {
            return res.status(400).json({ erro: 'Dados incompletos' });
        }
        const result = await database_1.default.query(`INSERT INTO banco_saldo (mes, ano, saldo_inicial, saldo_extrato)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (mes, ano)
       DO UPDATE SET saldo_inicial = $3, saldo_extrato = $4
       RETURNING *`, [mes, ano, saldo_inicial, saldo_extrato]);
...
exports.salvarSaldoMensal = salvarSaldoMensal;
// Obter saldo mensal
const obterSaldoMensal = async (req, res) => {
    try {
        const { mes, ano } = req.params;
        const result = await database_1.default.query('SELECT * FROM banco_saldo WHERE mes = $1 AND ano = $2', [mes, ano]);
        ...
        const transacoesResult = await database_1.default.query(`SELECT
       ...
       WHERE mes = $1 AND ano = $2`, [mes, ano]);
        ...
        await database_1.default.query('UPDATE banco_saldo SET saldo_final = $1 WHERE mes = $2 AND ano = $3', [saldoFinal, mes, ano]);
...
exports.obterSaldoMensal = obterSaldoMensal;
...
const criarTransacao = async (req, res) => {
    ...
    try {
        ...
        const transacaoResult = await client.query(`INSERT INTO banco_transacoes
       (mes, ano, tipo, categoria_id, descricao, valor, ratear_condominos, data_transacao, criado_por)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING *`, [mes, ano, tipo, categoria_id, descricao, valor, ratear_condominos, data_transacao, usuario_id]);
        ...
        if (tipo === 'debito' && ratear_condominos) {
            ...
            await client.query(`INSERT INTO despesas_condominio (mes, ano, categoria_id, descricao, valor, valor_por_apto)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (mes, ano, categoria_id, descricao)
         DO UPDATE SET valor = $5, valor_por_apto = $6`, [mes, ano, categoria_id, descricao, valor, valorPorApto]);
        }
...
const listarTransacoes = async (req, res) => {
    try {
        const { mes, ano } = req.params;
        const result = await database_1.default.query(`SELECT bt.*, cd.nome as categoria_nome, u.nome as criado_por_nome
       ...
       ORDER BY bt.data_transacao DESC, bt.criado_em DESC`, [mes, ano]);
...
const atualizarTransacao = async (req, res) => {
    ...
    try {
        ...
        const transacaoAntigaResult = await client.query('SELECT * FROM banco_transacoes WHERE id = $1', [id]);
        ...
        const result = await client.query(`UPDATE banco_transacoes
       SET tipo = $1, categoria_id = $2, descricao = $3, valor = $4,
           ratear_condominos = $5, data_transacao = $6
       WHERE id = $7
       RETURNING *`, [tipo, categoria_id, descricao, valor, ratear_condominos, data_transacao, id]);
        ...
        if (transacaoAntiga.ratear_condominos && transacaoAntiga.tipo === 'debito') {
            await client.query(`DELETE FROM despesas_condominio
         WHERE mes = $1 AND ano = $2 AND descricao = $3`, [transacaoAntiga.mes, transacaoAntiga.ano, transacaoAntiga.descricao]);
        }
        ...
        if (tipo === 'debito' && ratear_condominos) {
         ...
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (mes, ano, categoria_id, descricao)
         DO UPDATE SET valor = $5, valor_por_apto = $6`, [transacaoAntiga.mes, transacaoAntiga.ano, categoria_id, descricao, valor, valorPorApto]);
        }
        await client.query('COMMIT');
...
const excluirTransacao = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await database_1.default.query('DELETE FROM banco_transacoes WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ erro: 'Transação não encontrada' });
        }
...
const calcularSaldo = async (req, res) => {
    try {
        ...
        const saldoResult = await database_1.default.query('SELECT saldo_inicial FROM banco_saldo WHERE mes = $1 AND ano = $2', [mes, ano]);
...
const conferirSaldo = async (req, res) => {
    try {
        const { mes, ano } = req.params;
        ...
        const saldoCalc = await database_1.default.query(`SELECT
         bs.saldo_inicial,
       ...
       GROUP BY bs.saldo_inicial`, [mes, ano]);
        ...
        // Atualizar registro
        await database_1.default.query(`UPDATE banco_saldo
       SET saldo_final = $1, saldo_extrato = $2, conferido = $3
       WHERE mes = $4 AND ano = $5`, [saldoSistema, saldo_extrato, conferido, mes, ano]);
        res.json({
...
exports.conferirSaldo = conferirSaldo;
// Mudar mês de cobrança de uma transação
const mudarMesCobranca = async (req, res) => {
    const client = await database_1.default.connect();
    try {
        const { id } = req.params;
        ...
        const transacaoResult = await client.query('SELECT * FROM banco_transacoes WHERE id = $1', [id]);
        ...
        if (transacao.ratear_condominos && transacao.tipo === 'debito') {
            ...
            await client.query(`DELETE FROM despesas_condominio
         WHERE mes = $1 AND ano = $2 AND descricao = $3`, [transacao.mes, transacao.ano, transacao.descricao]);
            // Criar despesa no novo mês
            const valorPorApto = transacao.valor / 6;
            await client.query(`INSERT INTO despesas_condominio (mes, ano, categoria_id, descricao, valor, valor_por_apto)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (mes, ano, categoria_id, descricao)
         DO UPDATE SET valor = $5, valor_por_apto = $6`, [novo_mes, novo_ano, transacao.categoria_id, transacao.descricao, transacao.valor, valorPorApto]);
        }
        // Atualizar o mês/ano da transação
        const result = await client.query(`UPDATE banco_transacoes
       SET mes = $1, ano = $2
       WHERE id = $3
       RETURNING *`, [novo_mes, novo_ano, id]);
        await client.query('COMMIT');
        res.json(result.rows[0]);
    }
    catch (error) {
...
exports.mudarMesCobranca = mudarMesCobranca;
//# sourceMappingURL=bancoController.js.map