...
const nomeMes = (mes: number): string => {
...
const gerarPixPayload = (chave: string, nome: string, cidade: string, valor?: number): string => {
...
export const gerarBoletoPDF = async (req: Request, res: Response) => {
  try {
    ...
    const condominoResult = await pool.query(
    ...
    const gasResult = await pool.query(
      `SELECT * FROM leituras_gas WHERE mes = $1 AND ano = $2 AND apartamento = $3`,
      [mesNum, anoNum, aptoFormatado]
    );
    ...
    try {
      ...
      const qrCodeDataUrl = await QRCode.toDataURL(payload, { width: 110, margin: 1 });
...
export const gerarBoletoTeste = async (req: Request, res: Response) => {
  try {
    const doc = new PDFDocument({ size: 'A4', margin: 40 });
    res.setHeader('Content-Type', 'application/pdf');
    ...
    try {
      ...
      const qrCodeDataUrl = await QRCode.toDataURL(payload, { width: 120, margin: 1 });
      const qrCodeBuffer = Buffer.from(qrCodeDataUrl.split(',')[1], 'base64');
...
export const gerarTodosBoletos = async (req: Request, res: Response) => {
  try {
    ...
    for (let i = 1; i <= 6; i++) {
      ...
      const gasResult = await pool.query(
        `SELECT valor_total FROM leituras_gas WHERE mes = $1 AND ano = $2 AND apartamento = $3`,
        [mes, ano, apto]
      );
