...
export const listarCategorias = async (req: Request, res: Response) => {
  try {
    const result = await pool.query(
      'SELECT * FROM categorias_despesas WHERE ativo = true ORDER BY ordem'
    );
...
export const criarCategoria = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO categorias_despesas (nome, ordem) VALUES ($1, $2) RETURNING *',
      [nome, ordem || 999]
    );
...
export const atualizarCategoria = async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE categorias_despesas SET nome = $1, ordem = $2 WHERE id = $3 RETURNING *',
      [nome, ordem, id]
    );
...
// Desativar categoria
export const desativarCategoria = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      'UPDATE categorias_despesas SET ativo = false WHERE id = $1 RETURNING *',
      [id]
    );
...
export const listarDespesasCondominio = async (req: Request, res: Response) => {
  try {
...
export const salvarDespesaCondominio = async (req: Request, res: Response) => {
  try {
...
export const atualizarDespesaCondominio = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const result = await pool.query(
      `UPDATE despesas_condominio
       SET descricao = $1, valor = $2, valor_por_apto = $3
       WHERE id = $4
       RETURNING *`,
      [descricao, valor, valorPorApto, id]
    );
...
export const excluirDespesaCondominio = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      'DELETE FROM despesas_condominio WHERE id = $1 RETURNING *',
      [id]
    );
...
export const criarDespesaParcelada = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const despesaResult = await client.query(
      ...
      [descricao, valor_total, num_parcelas, valor_parcela, mes_inicio, ano_inicio]
    );
    ...
    for (let i = 1; i <= num_parcelas; i++) {
      await client.query(
        `INSERT INTO parcelas_cobradas (despesa_parcelada_id, mes, ano, parcela_numero, valor)
         VALUES ($1, $2, $3, $4, $5)`,
        [despesaId, mesAtual, anoAtual, i, valor_parcela]
      );
      ...
      await client.query(
        `INSERT INTO despesas_condominio (mes, ano, descricao, valor, valor_por_apto)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (mes, ano, categoria_id, descricao) DO NOTHING`,
        [mesAtual, anoAtual, `${descricao} (${i}/${num_parcelas})`, valorTotalParcela, valor_parcela]
      );
...
export const listarDespesasParceladas = async (req: Request, res: Response) => {
  try {
    const result = await pool.query(`
      SELECT dp.*,
...
export const atualizarStatusParcela = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { cobrado } = req.body;

    const result = await pool.query(
      'UPDATE parcelas_cobradas SET cobrado = $1 WHERE id = $2 RETURNING *',
      [cobrado, id]
    );
...
export const obterResumoMensal = async (req: Request, res: Response) => {
  try {
    const { mes, ano } = req.params;
    ...
    const gas = await pool.query(
      `SELECT apartamento, valor_total as valor_gas
       FROM leituras_gas
       WHERE mes = $1 AND ano = $2
